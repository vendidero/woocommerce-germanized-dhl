...
copy_dest_files() {
	...
	rsync ./ "$2"/ --recursive --delete --delete-excluded \
		...
		--exclude=auth.json \
		--exclude=woocommerce-germanized-dhl.zip \
		--exclude="zip-file/"
	status "Done copying files!"
	cd "$CURRENT_DIR" || exit
}

status "💃 Time to release WooCommerce Germanized DHL 🕺"
...
cd zip-file
zip -r ../woocommerce-germanized-dhl.zip ./
cd ..
rm -r zip-file

success "Done. You've built WooCommerce Germanized DHL! 🎉"