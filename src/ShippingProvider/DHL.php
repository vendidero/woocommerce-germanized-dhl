...
class DHL extends Auto {
	...
	protected function get_general_settings( $for_shipping_method = false ) {
		...
		$settings = array(
			...
			array( 'title' => _x( 'API', 'dhl', 'woocommerce-germanized-dhl' ), 'type' => 'title', 'id' => 'dhl_api_options', 'desc' => $connection_status_html ),
			...
			array(
				...
				'custom_attributes'	=> array( 'data-show_if_sandbox_mode' => '', 'autocomplete' => 'new-password' )
			),
	...
	protected function get_label_settings( $for_shipping_method = false ) {
		...
		$settings = array_merge( $settings, array(
			array( 'title' => _x( 'Retoure', 'dhl', 'woocommerce-germanized-dhl' ), 'type' => 'title', 'id' => 'dhl_retoure_options', 'desc' => sprintf( _x( 'Adjust handling of return shipments through the DHL Retoure API. Make sure that your %s contains DHL Retoure Online.', 'dhl', 'woocommerce-germanized-dhl' ), '<a href="' . esc_url( Package::get_geschaeftskunden_portal_url() ) . '">' . _x(  'contract', 'dhl', 'woocommerce-germanized-dhl' ) . '</a>' ) ),
			...
			$pointers = array(
				'pointers' => array(
					'day'          => array(
						...
						'next_trigger' => array(),
						'options'      => array(
							'content'  => '<h3>' . esc_html_x( 'Delivery day', 'dhl', 'woocommerce-germanized-dhl' ) . '</h3>' .
							              '<p>' . esc_html_x( 'Let your customers choose a delivery day (if the service is available at the customer\'s location) of delivery within your checkout.', 'dhl', 'woocommerce-germanized-dhl' ) . '</p>',
							'position' => array(
								'edge'  => 'left',
								'align' => 'left',
							),
					...
					'fee'          => array(
						...
						'options'      => array(
							'content'  => '<h3>' . esc_html_x( 'Fee', 'dhl', 'woocommerce-germanized-dhl' ) . '</h3>' .
							              '<p>' . esc_html_x( 'Optionally charge your customers an additional fee for preferred services like delivery day.', 'dhl', 'woocommerce-germanized-dhl' ) . '</p>',
							'position' => array(
								'edge'  => 'left',
								'align' => 'left',
							),
					...
					'location'          => array(
						...
						'next_trigger' => array(),
						'options'      => array(
							'content'  => '<h3>' . esc_html_x( 'Drop-off location', 'dhl', 'woocommerce-germanized-dhl' ) . '</h3>' .
							              '<p>' . esc_html_x( 'Allow your customers to send their parcels to a drop-off location e.g. a neighbor. This service is free of charge for DHL shipments.', 'dhl', 'woocommerce-germanized-dhl' ) . '</p>',
							'position' => array(
								'edge'  => 'left',
								'align' => 'left',
							),
