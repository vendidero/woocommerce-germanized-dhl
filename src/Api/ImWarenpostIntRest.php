		...
		$request_data = array(
			...
			'items'       => array(
				array(
					...
					'recipient'           => wc_gzd_shipments_substring( $recipient, 0, 30 ),
					'recipientPhone'      => $shipment->get_phone(),
					'recipientEmail'      => $shipment->get_email(),
					'addressLine1'        => wc_gzd_shipments_substring( $shipment->get_address_1(), 0, 30 ),
					'addressLine2'        => wc_gzd_shipments_substring( $shipment->get_address_2(), 0, 30 ),
					'city'                => $shipment->get_city(),
					'state'               => wc_gzd_shipments_substring( $this->clean_state( wc_gzd_dhl_format_label_state( $shipment->get_state(), $shipment->get_country() ) ), 0, 20 ),
					...
					'senderName'          => wc_gzd_shipments_substring( $sender_name, 0, 40 ),
					'senderAddressLine1'  => wc_gzd_shipments_substring( $shipment->get_sender_address_1(), 0, 35 ),
					'senderAddressLine2'  => wc_gzd_shipments_substring( $shipment->get_sender_address_2(), 0, 35 ),
